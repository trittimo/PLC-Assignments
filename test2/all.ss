...
(define-datatype expression expression?
   (do2-exp (list-of-exps scheme-value?) (test-exp scheme-value?))
...
(define (has-expansion? datum)
   (member (1st datum) '( or and cond let* begin case while do1 )))
...
(define (eval-exp exp env)
   (cases expression exp
      (do2-exp (list-of-exps test-exp)
         (letrec ((eval-all (lambda (exps)
               (if (not (null? exps)) (begin (eval-exp (car exps) env) (eval-all (cdr exps))))))
                  (loop (lambda ()
                     (eval-all list-of-exps)
                     (if (eval-exp test-exp env) (loop)))))
            (loop)))
      (if-exp (comp true false)
